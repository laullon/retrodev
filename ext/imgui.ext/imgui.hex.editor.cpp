...
namespace ImGui::HexEditor {
	...
	//
	void RenderByteDecorations(ImDrawList* draw_list, const ImRect& bb, ImColor bg_color, HighlightFlags flags, ImColor border_color, float rounding, int offset, int range_min,
							   int range_max, int bytes_per_line, int i, int line_base) {
					...
					if (offset >= select_start_byte && offset <= select_end_byte) {
						...
						if (state->SingleHighlightCallback) {
							...
							if (flags & HighlightFlags_Apply) {
								...
								RenderByteDecorations(draw_list, (flags & HighlightFlags_FullSized) ? item_bb : byte_bb, color, flags, highlight_border_color, style.FrameRounding,
						...
						if (!single_highlight) {
							for (int j = 0; j != state->HighlightRanges.Size; j++) {
								...
								if (line_base + i >= range.From && line_base + i <= range.To) {
									...
									RenderByteDecorations(draw_list, (range.Flags & HighlightFlags_FullSized) ? item_bb : byte_bb, range.Color, range.Flags, highlight_border_color,
